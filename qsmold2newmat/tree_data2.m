...
function treedata = tree_data2(cylinder,branch,inputs)
...
treedata = dbh_cylinder(treedata,Trunk,Rad,Len,ind);
...
if inputs.disp == 2
    ...
    Units = zeros(n,3);
    for i = 1:n
        if strcmp(Names{i},'CrownVolumeAlpha')
            m = i;
        end
...
function treedata = dbh_cylinder(...
    treedata,Trunk,Rad,Len,ind)
...
spreads = zeros(20,18);
for j = 1:20
    I = P(:,3) >= bot+(j-1)*Hei/20 & P(:,3) < bot+j*Hei/20;
    if any(I)
    X = unique(P(I,:),'rows');
    if size(X,1) > 2
...
% hold on
% plot(X(K,1),X(K,2),'-r')
% plot(CX,CY,'or','Markersize',10,'Linewidth',3)
