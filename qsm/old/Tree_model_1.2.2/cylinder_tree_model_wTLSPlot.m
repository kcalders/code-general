function [TreeData, Sta, Axe, Rad, Len, CPar, CExt, BoC, BOrd, BPar, ...
    ...
    = cylinder_tree_model_wTLSPlot(P, dmin, rcov, nmin, lcyl, NoGround, string,...
...
for i = 0:BO
    I = BOrd == i;
    BODist(i+1,1) = sum(BVol(I)); % volumes
    BODist(i+1,2) = sum(BLen(I)); % lengths
    BODist(i+1,3) = nnz(I); % number of ith-order branches
end
...
plot_branching_structure(BoC,Rad,Len,Axe,Sta,1,20,0.3)
hold on
%if TreeData(19) > 0
    %point_cloud_plotting(Trunk,1,5)
