function [Rad, Len, Axe, Sta, CPar, CExt, CChi, SoC, CiS, Segs, SPar, SChi] = ...
...
dmax = 0;
for i = 1:size(Bal,1)
    V = mat_vec_subtraction(P(Cen(Nei{i}),:),P(Cen(i),:));
    d = max(sum(V.*V,2));
    if d > dmax
        dmax = d;
    end
...
for si = 1:NumOfSeg
    if ~isempty(Segs{si})
        ...
        if (nc > 2) && (np > 20) && (~isempty(Base)) % analyse only large enough segments
            %% Determine the regions for cylinder fitting
            if nb == np   % Segment is only the base
                Regs = cell(1,1);
                Regs{1} = Q;
                Axes = direc;
                Starts = mean(P(Q,:));
                [d,~,h] = distances_to_line(P(Q,:),Axes,Starts);
                Lengs = max(h)-min(h);
                Rads = median(d);
                H = P(Q,:)*Axes';
                hpoint = Axes*Starts';
                ...
                t = 1;
                NL = ceil(Lengs/dmin);           
            else  % segment is larger than its base
                
                [Regs,Starts,Lengs,Axes,Rads,Mads,t,NL] = regions(P,Bal,Seg,direc,nl,Trunk,Cen,dmin);
                
                % Modify regions if very long cylinders
                if (si > 1) && (NL*2*dmin < dmax)
                    [m,i] = max(Lengs);
                    if m > 1.5*dmax && m/Rads(i) > 20
                        k = 1;
                        while k <= t
                            if Lengs(k) > 1.5*dmax
                                t = k-1;
            ...
            for j = 1:t
                if (length(Regs{j}) > 10) && (norm(Axes(j,:)) > 0) % fit cylinders to large enough subsegs
                    ...
                    AP0 = mean(Points)';   % point in the cylinder axis
                    R0 = Rads(j);
                    mad0 = Mads(j);
                    FQ(j,1) = mad0;
                    ...
                    % First fitting
                    [AP, CA, R, d, ~, conv, rel] = ...
                        lscylinder(Points, AP0, CA0, R0, 0.1, 0.1);
                                    
                    % Conditions for second fitting
                    I1 = conv == 1;   % fitting converged
                    I2 = rel == 1;    % reliable fitting, no very bad condition numbers 
                    mad = mean(abs(d));  % mean distance to the fitted cylinder
                    md = max(d);  % maximum distance
                    I3 = mad < R0;  % very large distances not acceptable
                    I4 = abs(CA0'*CA) > 0.5; % large angle not acceptable
                    I5 = mad > 0.02;  % large absolut distances
                    I6 = mad/R > 0.1;  % large relative distances
                    I7 = md/R > 0.333;  % large maximum relative distance
                    I8 = R > 2*R0;  % large difference in radii
                    I9 = R < 5*R0;
                    I10 = R > 0;
                    Accept = I1&I2&I3&I4&I9&I10; % accept the first fitting
                    Second = Accept & (I5|I6|I7|I8); % second cylinder fitting
                    ...
                    if Second
                        ...
                        [AP, CA, R, d, ~, conv, rel] = ...
                            lscylinder(Points, AP, CA, R, 0.1, 0.1);
                        ...
                        I5 = R < 5*R0;
                        I6 = R > 0;
                        Accept = I1&I2&I3&I4&I5&I6; % accept the second fitting
                    ...
                    if Accept && ((CovExt(1) >= 0.3125) || (CovExt(2) == 1) || (mad/R0 < 0.2))
                        Rads(j) = R;
                        Axes(j,:) = CA';
                        Lengs(j) = L;
                        Starts(j,:) = SP;
                        FQ(j,2:4) = [mad CovExt];
                    elseif I1 && I2 && I4 && (R/R0 < 2.5)
                        Rads(j) = R;
                        Axes(j,:) = CA';
                        Lengs(j) = L;
                        Starts(j,:) = SP;
            ...
            %(NL >= 9 && t > 1 && mean(Rads) > 0.02)
            %% "Refinement" fitting for "large" segments
            if (mean(Rads) > 0.02) && ((t > 1 && NL >= 3 && mean(Lengs(2:t))/mean(Rads(2:t)) > 8) || (Lengs0(1) > 5*dmin && Lengs0(1)/Rads(1) > 10 && t == 1))
                % Each region is devided into three parts where the
                % cylinders are fitted
                [Rads,Lengs,Starts,Axes,FQ,t] = ...
                    refinement_fitting(P,Regs,Rads,Lengs,Starts,Axes,FQ,t);            
            end
            
            
            %% Search possible parent cylinder
            if t > 0
                [PC,Starts,Lengs,Axes,Segs,SPar,SChi,CExt,t] = parent_cylinder...
                    (P,Regs,Segs,SPar,SChi,CiS,CExt,Rad,Axe,Sta,Len,Starts,Axes,Rads,Lengs,c,si,t);
            end
            ...
            if t > 0
                [Rads,Starts,Axes,Lengs,t] = adjustments(P,Rad,Sta,CiS,Rads,...
                    Starts,Axes,Lengs,Regs,FQ,Width,Height,Dt,Mt,PC,Trunk,si,t,SoC,SChi,SPar);
            end
            ...
            %% Check if some problems with very long cylinders
            [m,i] = max(Lengs);
            if (si > 1) && (t > 1) && ((NL*2*dmin < dmax) || (m/Rads(i) > 30))
                if m > 1.5*dmax && m/Rads(i) > 20
                    k = 2;
                    while k <= t
                        if Lengs(k) > 1.5*dmax
...
%% Plot the cylinders and the segments
%plot_tree_structure(P,Bal,Segs,SChi,2,3,0,1)
%hold on
%plot_cylinder_model(Rad,Len,Axe,Sta,2,30,0.2)
%hold off
% 
%plot_cylinder_model(Rad,Len,Axe,Sta,1,30,0.2)
%plot_tree(P,Bal,Segs,SChi,Rad,Sta,Len,Axe,CiS,6,3,1)

end % End of main function


function [Seg,Segment,Base,nl] = fix_segment...
    (P,Cen,Seg,Segment,Base,nl)
...
function [Regs,Starts,Lengs,Axes,Rads,Mads,t,NL] = regions...
    (P,Bal,Seg,direc,nl,Trunk,Cen,dmin)

% Define the regions for cylinder fitting

if nl > 3
    % #### Here the length of the cylinders are defined -->
    ...
    Regs = cell(1,1);
    Regs{1} = vertcat(Bal{vertcat(Seg{:})});
    Starts = mean(P(Regs{:},:));
...
function [Rads,Lengs,Starts,Axes,FQ,t] = refinement_fitting...
    (P,Regs,Rads,Lengs,Starts,Axes,FQ,t)
...
% hold on
% for j = 1:length(Rads)
%     draw(Rads(j),Lengs(j),Axes(j,:),Starts(j,:),1,30)
% end
% axis equal
% hold off
% alpha(0.5)
...
axes = zeros(3*t,3);
angles = zeros(3*t,1);
fq = zeros(3*t,4);  % fit quality for refined cylinders
for i = 1:t
    ...
    for j = 1:3
        ...
        if nnz(J) > 10
            ...
            fq(3*(i-1)+j,2:4) = [mad CovExt];
            %disp([conv rel r R mad mad0 A*a])
            if (conv == 0) || (rel == 0) || (r > 1.2*R && mad0 < 2*mad) || (r > 1.5*R) || (r <= 0)
                % Reject the fitting
                r = R;
                a = A';
                s = AP(j,:)';
    ...
    I = axe*A' < 0.35;
    %disp([axe*A'; sum(len); 1.2*L])
    if any(I) || sum(len) > 1.2*L
        rads(3*i-2:3*i) = r;
        angles(3*i-2:3*i) = abs(axe*A');
    else
        rads(3*i-2:3*i) = rad;
        lengs(3*i-2:3*i) = len;
        starts(3*i-2:3*i,:) = sta;
        axes(3*i-2:3*i,:) = axe;
        angles(3*i-2:3*i) = abs(axe*A');
    end
end

if sum(lengs) > 1.2*sum(Lengs)
    % too long
    %disp('too long')
elseif sum(angles) == 3*t
    % no changes
    %disp('no changes')
else
    I = true(3*t,1);
    for i = 1:t
        if (rads(3*i-2) == rads(3*i-1)) && (rads(3*i-2) == rads(3*i))
            I(3*i-1:3*i) = false;
            lengs(3*i-2) = Lengs(i);
            starts(3*i-2,:) = Starts(i,:);
            axes(3*i-2,:) = Axes(i,:);
            fq(3*i-2,:) = FQ(i,:);
        end
    end
...
% hold on
% for j = 1:length(Rads)
%     draw(Rads(j),Lengs(j),Axes(j,:),Starts(j,:),1,50)
% end
% axis equal
% hold off
% alpha(0.2)
...
function [PC,Starts,Lengs,Axes,Segs,SPar,SChi,CExt,t] = parent_cylinder...
    (P,Regs,Segs,SPar,SChi,CiS,CExt,Rad,Axe,Sta,Len,Starts,Axes,Rads,Lengs,c,si,t)
...
if SPar(si) > 0 % parent segment exists, find the parent cylinder
    ...
    if ~isempty(PC) 
        ...
        %% Check if a small segment and its cylinder are part of its parent segment
        ...
        if (SPar(si) > 0) && ~ParentFound && (t <= 3)
            ...
            while (j <= n) && ~ParentFound
                ...
                mh = mean(h);
                %disp([mean(d) 1.1*Rad(c(j)) mh Len(c(j))])
        ...
        while (j <= n) && ~ParentFound
            ...
            if (D(j,1) < R) && (D(j,2) < R) && (H(j,1) > 0) && (H(j,1) < L) && (H(j,2) > 0) && (H(j,2) < L)
        ...
        while (j <= n) && ~ParentFound
            R = Rad(pc(j));
            L = Len(pc(j));
            if (D(j,1) < R) && (D(j,2) < R+Rads(1)) && (H(j,1) > 0) && (H(j,1) < L) && (H(j,2) > 0) && (H(j,2) < L)
            ...
            elseif (t == 1) && (abs(Axe(pc(j),:)*Axes(1,:)') > 0.7) && ((D(j,1) < R+Rads(1)) || (D(j,2) < R+Rads(1))) ...
                    && (((H(j,1) > 0) && (H(j,1) < L)) || ((H(j,2) > 0) && (H(j,2) < L)))
        ...
        if ~ParentFound
            ...
            while j <= n && ~ParentFound
                if x(j,1) > 0 && x(j,2) < 0 && h(j,1) > 0 && h(j,1) < Len(pc(j))
                    ...
                    PC = pc(j);
                    if t == 1 && Rads(1) < 0.05*Rad(PC) && (Lengs(1)-x(j,1))/Rads(1) < 0.5
...
if SPar(si) == 1 && ~isempty(PC)
    rMod = Rads > 0.8*MaxR;
    Rads(rMod) = 0.8*MaxR;
elseif (t == 1) && (Rads(1) >= 4*MaxR) 
    if isempty(SChi(si))
        t = 0;
    else
        rMod = Rads(1) > MaxR;
...
elseif (t == 1) && (Rads(1) >= 2.5*MaxR) && (FQ(1,3) < 0.5) && (FQ(1,4) == 0)
    rMod = Rads(1) > MaxR;
    Rads(rMod) = MaxR;
elseif (t == 1) && (Rads(1) >= 2.5*MaxR) && (FQ(1,2)/Rads(1) > 0.1) && (FQ(1,4) == 0)
    rMod = Rads(1) > MaxR;
    Rads(rMod) = MaxR;
elseif t > 0
    if t == 1;
        rMod = Rads(1) > 1.2*MaxR;
        Rads(rMod) = MaxR;
    elseif ~isempty(PC) && SoC(PC) == 1
        rMod = Rads > 1.2*MaxR;
        Rads(rMod) = MaxR;
    else
        %rMod = Rads(1:t) > 1.2*MaxR;
        I = Rads(1:t) > MaxR;
        J = FQ(1:t,4) == 0;
        K = FQ(1:t,3) < 0.3125;
        L = Rads(1:t) > 3*MaxR;
...
if t > 1
    ...
    while (j <= t)
        ...
        elseif (I1 || I2) && I3 && I4 && I5 && I6
            k = 1;
            while (j+k <= t) && ((Rads(j+k) > 1.25*Rads(j-1)) || ((FQ(j+k,3) < 0.3125) && (FQ(j+k,2) > FQ(j-1,2)) && (FQ(j+k,3) < FQ(j-1,3)) && (FQ(j+k,4) == 0)))
                k = k+1;
            end
...
if Trunk && t > 2 
    if Rads(1) < 0.9*Rads(2)
        Rads(1) = Rads(2);
        Starts(1,:) = Starts(2,:)-Lengs(1)*Axes(2,:);
        Axes(1,:) = Axes(2,:);
    elseif Rads(2) < 0.9*Rads(3) 
        Rads(2) = Rads(3);
        Rads(1) = Rads(3);
        Starts(2,:) = Starts(3,:)-Lengs(2)*Axes(3,:);
        Starts(1,:) = Starts(2,:)-Lengs(1)*Axes(3,:);
        Axes(2,:) = Axes(3,:);
        Axes(1,:) = Axes(3,:);
    end
end
