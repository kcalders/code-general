function [Vtot,DiamSummary,Diameters,Vert,Facets,fvd,Normals,Centers,Areas]...
    = triangulated_cylinder_surface(P,NA,H,CL,maxL,Axe,Sta)
...
if m > 0
    ...
    Vtot = sum(A1.*sum(C1.*N1,2))+sum(A2.*sum(C2.*N2,2))+sum(A3.*sum(C3.*N3,2))+sum(A4.*sum(C4.*N4,2));
    Vtot = 1000*Vtot/3;
    if Vtot > 100
        Vtot = round(Vtot);
    elseif Vtot > 10
...
Facets = [Tria(:,1) Tria(:,3) Tria(:,4); Tria(:,1) Tria(:,4) Tria(:,2); Bot(:,1) Bot(:,2) Bot(:,3); Top(:,1) Top(:,2) Top(:,3)];
fvd = ones(size(Facets,1),1);

% figure(1)
% plot3(P(:,1),P(:,2),P(:,3),'.c','Markersize',5)
% patch('Vertices',Vert,'Faces',Facets,'FaceVertexCData',fvd,'FaceColor','flat')
% %hold on
% %quiver3(Centers(:,1),Centers(:,2),Centers(:,3),Normals(:,1),Normals(:,2),Normals(:,3),'.m','Linewidth',4)
% %plot_cylinder_model(Rad(1:n),Len(1:n),Axe(1:n,:),Sta(1:n,:),1,10,0.2)
% %hold off
% axis equal
% alpha(0.4)