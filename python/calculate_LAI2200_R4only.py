...
#  	print file_list
	...
	Pgap_ps = np.asarray(Pgap_ps)
 	...
 	#CALCULATE PAIe
  	PAIe=2*(-math.log(Pgap_ps_avg[3])/dist4*w4)
	return PAIe
	
def calculatePAIe_m(p,s,m):
	"""
	Calculates the PAIe for each VALERI plot
	PAIe = clumping index X PAI 
	See Ryu et. al (AFM, 2010) on the correct estimation of effictive leaf area index.
	...
	In this script p=plot, s=subplot (i.e. one VALERI plot)
	"""
	print "Processing PAIe VALERI plot P%iS%iM%i" % (p,s,m)
	

	dist1=1.008;dist2=1.087;dist3=1.270;dist4=1.662;dist5=2.670 #values from LICOR2200 manual
	w4=1

  	Pgap_psm= np.genfromtxt("output/Pgap_P%iS%iM%i_avg.csv" %(p,s,m), delimiter=',', skip_header=1)
	Pgap_psm = np.asarray(Pgap_psm)


 	
 	#CALCULATE PAIe
  	PAIe=2*(-math.log(Pgap_psm[3])/dist4*w4)
	return PAIe
	
def getPgap_m_R4(p,s,m):
  	Pgap_psm= np.genfromtxt("output/Pgap_P%iS%iM%i_avg.csv" %(p,s,m), delimiter=',', skip_header=1)
	Pgap_psm = np.asarray(Pgap_psm)
	return Pgap_psm[3]
...
def main(cmdargs):	
 	...
 	PAI_VALERI=np.hstack([VALERI,PAIe_vals])
 	np.savetxt("SUMMARY_PAI_VALERI_R4.csv", PAI_VALERI, header='VALERI_PLOT,PAIe', fmt="%s", delimiter=",")
	...
	summary=([]) 	
  	for p in (1,2,5,6,7,8):
 		for s in range(1,26):
 			for m in (1,4,7,10):
# 			for m in range(1,13):
				PAIe_vals.append([calculatePAIe_m(p,s,m)])
				Pgap_vals.append([getPgap_m_R4(p,s,m)])
				summary.append(["P%iS%iM%i" %(p,s,m)])
 	PAI_summary=np.hstack([summary,Pgap_vals,PAIe_vals])
 	np.savetxt("SUMMARY_PAI_INDIVIDUAL_R4_ALL2.csv", PAI_summary, header='LOCATION,Pgap,PAIe', fmt="%s", delimiter=",") 	
